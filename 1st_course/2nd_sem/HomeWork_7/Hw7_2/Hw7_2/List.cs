...
namespace Hw72
{
    ...
    public class List<T> : IEnumerable
    {
        ...
        /// <summary>
        /// Находит позицию искомого элемента в списке, если его нет то выдает -1;
        /// </summary>
        public int FindPositionOfElementInList(T value)
        {
            if(IsContained(value))
            {
                var cursor = head;
                int position = 0;
                while (!Equals(value, cursor.Value))
                {
                    position++;
                    cursor = cursor.Next;
                }
        ...
        /// </summary>
        public void DeleteElementFromList(T value)
        {
            if(IsContained(value))
            {
        ...
        /// </summary>
        public bool IsContained(T value)
        {
            ListElement cursor = head;
            while (cursor != null)
            {
                if (Equals(value, cursor.Value))
                {
                    return true;
                }
                cursor = cursor.Next;
