...
namespace Kr1
{
    ...
    public class List<T> : IEnumerable<T>
    {
        ...
        /// <summary>
        /// Считает кол-во нулевых обьектов в списке
        /// </summary>
        /// <param name="nullElement">"Элемент, который считается нулевым</param>
        /// <returns>Кол-во нулевых элементов</returns>
        public int CountNullElements(NullElementList nullElementList)
        {
            if(head == null)
            {
                return 0;
            }
            object nullElement = nullElementList.GetNullForThisType(head.Value);
            int countOfNull = 0;
            ListElement cursor = head;
            for (int i = 0; i < GetLength(); i++)
            {
                if (Equals(cursor.Value, nullElement))
                {
                    countOfNull++;                 
                }
                cursor = cursor.Next;
